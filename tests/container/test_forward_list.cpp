...
TEST(forward_list, push_back_1) {
	...
	flist_push_back(flist, &x);
	ASSERT_NE(flist, nullptr);
	ASSERT_NE(flist->head, nullptr);
	ASSERT_NE(flist->tail, nullptr);
	ASSERT_EQ(flist->head, flist->tail);

	auto *data = (struct vec2 *)flist->tail->data;
	ASSERT_NE(data, nullptr);
	ASSERT_EQ(data->x, x.x);
	ASSERT_EQ(data->y, x.y);
...
TEST(forward_list, push_back_2) {
	...
	flist_push_back(flist, &x);
	ASSERT_NE(flist, nullptr);
	ASSERT_NE(flist->head, flist->tail);
